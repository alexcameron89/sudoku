...
impl Solver {
    ...
    fn clone_and_set(&self, row: i32, column: i32, value: i32) -> Solver {
        let synthetic_square = Square {
            ...
            column: column.abs(),
            set: true,
            possible_values: [value, 0, 0, 0, 0, 0, 0, 0, 0],
        };
...
mod tests {
    ...
    fn it_keeps_track_of_unsolved_squares() {
        let grid = vec![
            ...
            let unsolved = vec![
                Square { row: 0, column: 0, possible_values: [1,2,3,4,5,6,7,8,9], set: false },
                Square { row: 1, column: 1, possible_values: [1,2,3,4,5,6,7,8,9], set: false }
            ];
